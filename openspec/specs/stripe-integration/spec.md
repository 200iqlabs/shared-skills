...
The system SHALL provide `skills/cfo/scripts/get_mrr.py` that calculates Monthly Recurring Revenue by summing normalized amounts from all active subscription items. The script SHALL normalize different billing intervals (day, week, month, year) to monthly amounts.
...
The system SHALL provide `skills/cfo/scripts/get_revenue.py` that fetches paid invoices for a date range via `GET /v1/invoices` and summarizes revenue by currency and month.
